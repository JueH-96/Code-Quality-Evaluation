#!/usr/bin/env python3
"""
Bandit Code-Level Correlation Analyzer
Analyzes Pearson correlations between Bandit security results for codes under the same LLM
Analyzes by difficulty classification and includes code correctness correlations
...
class BanditCorrelationAnalyzer:
    def __init__(self, bandit_result_dir="bandit_result", output_dir="bandit_code_level_result"):
        """
        Initialize correlation analyzer
        
        Args:
            bandit_result_dir: Bandit results directory
            output_dir: output directory
        """
        self.bandit_result_dir = bandit_result_dir
    ...
    def _normalize_columns_by_total_codes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Columns to potentially normalize - Bandit security metrics
        columns_to_check = [
            'Total Issues',
            'High Severity',
            'Medium Severity',
    ...
    def calculate_correlations(self, df: pd.DataFrame) -> Dict:
        ...
        Includes Passed Tests and security metrics analysis
        ...
        # Select numeric columns for analysis - Bandit security metrics
        numeric_columns = [
            'Passed Tests',
            'Total Issues',
            'High Severity',
    ...
    def analyze_llm(self, llm_name: str) -> Dict:
        ...
        all_file = os.path.join(self.bandit_result_dir, f"{llm_name}_Code_Bandit.csv")
        ...
        for difficulty in ['easy', 'medium', 'hard']:
            difficulty_file = os.path.join(
                self.bandit_result_dir, 
                f"{llm_name}_{difficulty.capitalize()}_Code_Bandit.csv"
            )
    ...
    def run_batch_analysis(self):
        ...
        """
        print("=" * 60)
        print("Starting Bandit Code-Level Correlation Analysis")
        print("(With normalization by row count and accuracy statistics)")
        print("=" * 60)
        
        # Check if bandit results directory exists
        if not os.path.exists(self.bandit_result_dir):
            print(f"Error: {self.bandit_result_dir} directory not found")
            return
        
        # Find all *_Code_Bandit.csv files (overall files)
        pattern = os.path.join(self.bandit_result_dir, "*_Code_Bandit.csv")
        all_files = glob.glob(pattern)
        
        if not all_files:
            print(f"Error: No *_Code_Bandit.csv files found in {self.bandit_result_dir}")
            return
        ...
        for file_path in all_files:
            ...
            # Remove "_Code_Bandit.csv" suffix
        ...
        # Save results
        output_file = os.path.join(self.output_dir, "bandit_code_level_summary.csv")
...
def main():
    """Main function"""
    print("\n" + "=" * 60)
    print("Bandit Code-Level Correlation Analysis Tool")
    print("(Enhanced Version with Full Names and Accuracy Stats)")
    print("=" * 60)
    print("\nFeatures:")
    print("  This tool analyzes Pearson correlations between Bandit security results")
    ...
    print("\nPrerequisites:")
    print("  1. Run bandit_checker.py first to generate bandit_result/ directory")
    print("  2. bandit_result/ directory should contain files:")
    print("     - <LLM>_Code_Bandit.csv (overall data)")
    print("     - <LLM>_Easy_Code_Bandit.csv (Easy difficulty)")
    print("     - <LLM>_Medium_Code_Bandit.csv (Medium difficulty)")
    print("     - <LLM>_Hard_Code_Bandit.csv (Hard difficulty)")
    print("\nOutput files:")
    print("  - bandit_code_level_result/bandit_code_level_summary.csv")
    print("  - bandit_code_level_result/bandit_code_level_summary.xlsx (if openpyxl installed)")
    print("")
    
    analyzer = BanditCorrelationAnalyzer(
        bandit_result_dir="bandit_result",
        output_dir="bandit_code_level_result"
    )
    ...
    if result:
        ...
        print(f"  - Passed Tests vs all security metrics (code correctness vs security)")
        print(f"  - Total Issues vs Severity levels (High/Medium/Low)")
        print(f"  - Severity level inter-correlations")
        print(f"  - Confidence level inter-correlations (High/Medium/Low)")
        print(f"  - Severity vs Confidence cross-correlations")
        print(f"\nKey Research Questions:")
        print(f"  - Does code correctness correlate with security issue count?")
        print(f"  - Are high-severity issues related to high-confidence detections?")
        print(f"  - Do different severity levels correlate with each other?")
        print(f"  - Is there a pattern between confidence and severity?")
    else:
        print("\nPlease run bandit_checker.py first to generate bandit_result directory and data files")
