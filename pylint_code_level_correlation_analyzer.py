#!/usr/bin/env python3
"""
Pylint Code-Level Correlation Analyzer
Analyzes Pearson correlations between pylint results for codes under the same LLM
Analyzes by difficulty classification and includes test pass rate correlations
- Uses full column names instead of abbreviations
- Calculates accuracy statistics
- Normalizes columns by total row count (number of codes) before correlation analysis
"""
...
class PylintCorrelationAnalyzer:
    def __init__(self, pylint_result_dir="pylint_result", output_dir="pylint_code_level_result"):
        """
        Initialize correlation analyzer
        
        Args:
            pylint_result_dir: pylint results directory
            output_dir: output directory
        """
        self.pylint_result_dir = pylint_result_dir
    ...
    def _convert_passed_tests_to_numeric(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def parse_passed_tests(value):
            ...
            # Format: "Yes"/"No" (primary format in your data)
    ...
    def _normalize_columns_by_total_codes(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Normalize columns by dividing by total number of rows (codes) to get averages
        ...
        print(f"    Total codes (rows): {total_codes}")
        
        # Make a copy to avoid modifying original
        df_normalized = df.copy()
        
        # Columns to potentially normalize
        columns_to_check = [
            'Total Issues',
            'Convention (C)',
            'Refactor (R)',
        ...
        for col in columns_to_check:
            ...
            if col in df.columns and 'avg' not in col.lower():
                try:
                    # Divide by total codes to get average per code
                    df_normalized[col] = df[col] / total_codes
                    normalized_count += 1
                    print(f"    Normalized column: {col} (divided by {total_codes})")
                except Exception as e:
                    print(f"    Warning: Failed to normalize {col}: {e}")
        
        if normalized_count > 0:
            print(f"    Total {normalized_count} columns normalized by row count")
        else:
    ...
    def calculate_correlations(self, df: pd.DataFrame) -> Dict:
        ...
        Includes Passed Tests analysis, all codes participate in statistics
        Columns are first normalized by total row count (except those containing 'avg')
        ...
        # Calculate accuracy statistics first (before normalization)
        accuracy_stats = self.calculate_accuracy_stats(df)
        print(f"    Accuracy: {accuracy_stats.get('Accuracy_Percent', 'N/A')} ({accuracy_stats.get('Passed_Count', 0)}/{accuracy_stats.get('Total_Codes', 0)})")
        
        # Normalize columns by Total Codes before correlation calculation
        df = self._normalize_columns_by_total_codes(df)
        
        # Select numeric columns for analysis - including Passed Tests
        numeric_columns = [
            'Passed Tests',
            'Total Issues',
            'Convention (C)',
    ...
    def analyze_llm(self, llm_name: str) -> Dict:
        ...
        all_file = os.path.join(self.pylint_result_dir, f"{llm_name}_Code_Pylint.csv")
        ...
        for difficulty in ['easy', 'medium', 'hard']:
            difficulty_file = os.path.join(
                self.pylint_result_dir, 
                f"{llm_name}_{difficulty.capitalize()}_Code_Pylint.csv"
            )
    ...
    def run_batch_analysis(self):
        ...
        """
        print("=" * 60)
        print("Starting Pylint Code-Level Correlation Analysis")
        print("(With normalization by row count and accuracy statistics)")
        print("=" * 60)
        
        # Check if pylint results directory exists
        if not os.path.exists(self.pylint_result_dir):
            print(f"Error: {self.pylint_result_dir} directory not found")
            return
        
        # Find all *_Code_Pylint.csv files (overall files)
        pattern = os.path.join(self.pylint_result_dir, "*_Code_Pylint.csv")
        all_files = glob.glob(pattern)
        
        if not all_files:
            print(f"Error: No *_Code_Pylint.csv files found in {self.pylint_result_dir}")
            return
        ...
        for file_path in all_files:
            ...
            # Remove "_Code_Pylint.csv" suffix
        ...
        # Save results
        output_file = os.path.join(self.output_dir, "pylint_code_level_summary.csv")
...
def main():
    """Main function"""
    print("\n" + "=" * 60)
    print("Pylint Code-Level Correlation Analysis Tool")
    print("(Enhanced Version with Full Names and Accuracy Stats)")
    print("=" * 60)
    print("\nFeatures:")
    print("  This tool analyzes Pearson correlations between pylint results")
    ...
    print("  ** Columns not containing 'avg' are normalized by row count **")
    print("\nPrerequisites:")
    print("  1. Run pylint_checker.py first to generate pylint_result/ directory")
    print("  2. pylint_result/ directory should contain files:")
    print("     - <LLM>_Code_Pylint.csv (overall data)")
    print("     - <LLM>_Easy_Code_Pylint.csv (Easy difficulty)")
    print("     - <LLM>_Medium_Code_Pylint.csv (Medium difficulty)")
    print("     - <LLM>_Hard_Code_Pylint.csv (Hard difficulty)")
    print("\nOutput files:")
    print("  - pylint_code_level_result/pylint_code_level_summary.csv")
    print("  - pylint_code_level_result/pylint_code_level_summary.xlsx (if openpyxl installed)")
    print("")
    
    analyzer = PylintCorrelationAnalyzer(
        pylint_result_dir="pylint_result",
        output_dir="pylint_code_level_result"
    )
    ...
    if result:
        ...
        print(f"  - N_Codes: Number of codes included in analysis")
        print(f"  - Accuracy: Code correctness rate (pass rate)")
        print(f"  - ** Full column names used throughout (e.g., 'Passed Tests vs Total Issues') **")
        print(f"  - ** Values are normalized (averaged) by row count before correlation **")
        ...
        print(f"  - Passed Tests vs all other metrics (code correctness correlations)")
        print(f"  - Total Issues vs other metrics")
        print(f"  - Inter-correlations between issue types (Convention, Refactor, Warning, Error, Fatal)")
    else:
        print("\nPlease run pylint_checker.py first to generate pylint_result directory and data files")
